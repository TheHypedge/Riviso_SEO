...
async def execute_structural_humanization(
    *,
    ...
    for pass_num in range(max(1, max_passes)):
        ...
        if final_ai <= _TARGET_AI_PCT:
            break
        if pass_num >= max_passes - 1:
            break
        strength = min(0.9, strength + 0.03)
    ...
    if final_ai > _TARGET_AI_PCT:
        for i in range(len(current)):
            current[i] = _deep_polish(current[i], protected_terms=protected_terms, strength=0.85)
        joined = join_markdown_paragraphs(current)
        final_ai = float((auditor.audit_markdown(joined)).get("ai_percentage") or final_ai)
    ...
    if final_ai > _TARGET_AI_PCT:
        ...
        for i in touch or list(range(len(current))):
            if 0 <= i < len(current):
                current[i] = _deep_polish(current[i], protected_terms=protected_terms, strength=0.88)
        joined = join_markdown_paragraphs(current)
        final_ai = float((auditor.audit_markdown(joined)).get("ai_percentage") or final_ai)
