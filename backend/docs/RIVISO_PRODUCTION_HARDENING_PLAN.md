...
| ID | Item | Target | Status |
|----|------|--------|--------|
| **L6.1** | Load test @ 15 concurrent users / realistic generation mix | p95 latency within target, no errors | ✅ **Run 2026-08-04 against production**, with user go-ahead. `http_req_duration` p95=204ms (target <1000ms), error rate 0.31% (target <1%), zero 5xx. All k6 thresholds passed. Run used a read-only variant (generation scenario disabled — running real `/generate` calls would spend live OpenAI budget and contend with real customers for the worker's 3-slot concurrency budget, not authorized for this pass); the 27 failed requests were all `429`s from S1.9/S1.10 per-user rate limiting correctly firing because the test script shares one login across all 15 VUs by design — confirms the limiter works, not a defect. Test fixture (synthetic user/project/article) created and fully removed afterward via the real `purge_user_data()` GDPR-purge path — zero residue in production data. Full-mix load test *with* real generation traffic (needs explicit OpenAI-cost authorization) still not done. |
